...
if(!$action){
	...
	$creatadd = "WHERE ifcheck=1 AND t.special NOT IN(2,3)";
	...
	if($db_tlist){
		...
		foreach($tlistdb as $key=>$val){
			$key>0 && $ttable_a[] = (int)$key;
		}
		$ttable_a[] = '';
	}
	if(isset($ttable_a[$ttable])){
		$pw_tmsgs = 'pw_tmsgs'.$ttable_a[$ttable];
		list($tidmin,$tidmax)=maxmin($ttable_a[$ttable]);
		$creatadd.= " AND t.tid>'$tidmin'".($tidmax>0 ? " AND t.tid<='$tidmax'" : '');
	} else{
		$pw_tmsgs = 'pw_tmsgs';
	}
	...
	$next=$start+$percount;
	$step++;
	$j_url="$basename&action=$action&percount=$percount&creatfid=$creatfid&fids=$fids";
	$goon=0;
	if($creatfid=='all'){
		$fids = $ext = '';
		...
		if($fids){
			$creatadd .= " AND t.fid IN('".str_replace(",","','",$fids)."')";
			unset($fids);
		} else{
			adminmsg('template_noforum');
		}
	} elseif(is_numeric($creatfid)){
		$creatadd .= " AND t.fid='$creatfid'";
	} else{
	...
	$ptable_a[] = 'pw_posts';
	if($db_plist){
		$plistdb = explode(',',$db_plist);
		foreach($plistdb as $key=>$val){
			$val>0 && $ptable_a[] = 'pw_posts'.$val;
		}
		$pw_posts = isset($ptable_a[$ptable]) ? $ptable_a[$ptable] : 'pw_posts';
		$creatadd .= " AND t.ptable='".(int)str_replace('pw_posts','',$pw_posts)."'";
	}
	...
	if($goon){
		$j_url .= "&step=$step&ttable=$ttable&ptable=$ptable";
		adminmsg('updatecache_step',EncodeUrl($j_url));
	} else{
		$ptable++;
		if(isset($ptable_a[$ptable])){
			$j_url .= "&step=1&ttable=$ttable&ptable=$ptable";
			adminmsg('updatecache_step1',EncodeUrl($j_url));
		}
		$ttable++;
		if(isset($ttable_a[$ttable])){
			$j_url .= "&step=1&ttable=$ttable";
			adminmsg('updatecache_step1',EncodeUrl($j_url));
		}
		adminmsg('operate_success');
	}
