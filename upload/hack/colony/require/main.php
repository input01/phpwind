...
if($job=="album"){
	...
	if($db_ifftp){
		...
		$ftp->delete("cn_img/$cydb[cnimg]");
	}else{
		P_unlink("$attachdir/cn_img/$cydb[cnimg]");
	}
	...
	if(!$step){
		...
		if($attachment){
			...
			$fileuplodeurl = "cn_img/colony_$cyid.$attach_ext";
			$filename = $db_ifftp ? $db_ftpweb."/".$fileuplodeurl : $attachdir.'/'.$fileuplodeurl;

			if($db_ifftp){
				$db_attachdir=1;$savedir='cn_img';
				require_once(R_P.'require/ftp.php');
				$ftp->upload($attachment,$fileuplodeurl);
			}elseif(!postupload($attachment,$filename)){
				Showmsg('colony_uploadfail');
			}
		...
		if(function_exists('getimagesize') && $cnimg && !(list($width,$height)=getimagesize($filename))){
			$db_ifftp ? $ftp->delete($fileuplodeurl) : P_unlink($filename);
