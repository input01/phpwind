...
if($action=="receivebox"){
	...
	*/
	...
	$pmsgdb = array();
	$newread= $newdel = '';
	$query  = $db->query("SELECT mid,fromuid,togroups,username,type,title,mdate FROM pw_msg WHERE type='public' AND togroups LIKE '%,$msg_gid,%' AND mdate>'$winddb[regdate]' ORDER BY mdate DESC");
	...
	$msgstart = $msgcount > $gp_maxmsg ? $msgcount-$gp_maxmsg : 0;
	$msgtoall = $msgcount > $gp_maxmsg ? $gp_maxmsg : $msgcount;
	(!is_numeric($page) || $page<1 || $gp_maxmsg<=$permsg) && $page = 1;
	if($msgtoall>$permsg){
		require_once(R_P.'require/forum.php');
		$numofpage = ceil($msgtoall/$permsg);
		$page > $numofpage && $page = $numofpage>0 ? $numofpage : 1;
		$msgstart += ($page-1)*$permsg;
		$pages = numofpage($msgtoall,$page,$numofpage,"message.php?action=$action&");
	}
	$query = $db->query("SELECT mid,fromuid,touid,username,ifnew,title,mdate FROM pw_msg WHERE type='rebox' AND touid='$winduid' ORDER BY mdate DESC LIMIT $msgstart,$permsg");
	while($msginfo=$db->fetch_array($query)){
		$msginfo['title']=substrs($msginfo['title'],35);
		$msginfo['mdate']=get_date($msginfo['mdate']);
		$msginfo['from']=$msginfo['username'];
	...
	*/
	...
	(!is_numeric($page) || $page<1 || $gp_maxmsg<=$permsg) && $page = 1;
	if($gp_maxmsg>$permsg){
		$rt = $db->get_one("SELECT COUNT(*) AS sum FROM pw_msg WHERE type='sebox' AND fromuid='$winduid'");
	...
	$query = $db->query("SELECT mid,fromuid,touid,username,ifnew,title,mdate FROM pw_msg WHERE type='sebox' AND fromuid='$winduid' ORDER BY mdate DESC LIMIT $msgstart,$permsg");
	while($msginfo=$db->fetch_array($query)){
		$msginfo['title'] = substrs($msginfo['title'],35);
		$msginfo['mdate'] = get_date($msginfo['mdate']);
		$msginfo['from']  = $windid;
	...
	*/
	...
	(!is_numeric($page) || $page<1 || $gp_maxmsg<=$permsg) && $page = 1;
	if($gp_maxmsg>$permsg){
		$rt = $db->get_one("SELECT COUNT(*) AS sum FROM pw_msg WHERE type='rebox' AND fromuid='$winduid'");
		$numofpage = ceil($rt['sum']/$permsg);
		require_once(R_P.'require/forum.php');
		$page > $numofpage && $page = $numofpage>0 ? $numofpage : 1;
		$msgstart += ($page-1)*$permsg;
	...
	$query = $db->query("SELECT mid,fromuid,touid,ms.username,me.username AS toname,ifnew,title,mdate FROM pw_msg ms LEFT JOIN pw_members me ON me.uid=ms.touid WHERE type='rebox' AND fromuid='$winduid' ORDER BY mdate DESC LIMIT $msgstart,$permsg");
	while($msginfo=$db->fetch_array($query)){
		$msginfo['title']=substrs($msginfo['title'],35);
		$msginfo['mdate']=get_date($msginfo['mdate']);
		$msginfo['from']=$windid;
...
function getusermsg($winduid,$ifnew,$type=''){
	...
	if($type==''){
		...
		global $msg_gid,$readmsg,$delmsg,$winddb;
		$checkmsg = '0';
		$readmsg && $checkmsg .=','.$readmsg;
		$delmsg  && $checkmsg .=','.$delmsg;
		$rt=$db->get_one("SELECT mid FROM pw_msg WHERE type='public' AND togroups LIKE '%,$msg_gid,%' AND mid NOT IN ($checkmsg) AND mdate>'$winddb[regdate]' LIMIT 1");
		if(!$rt){
			if($ifnew==3 || $ifnew==2){
				$ifnew-=2;
				$db->update("UPDATE pw_members SET newpm='$ifnew' WHERE uid='$winduid'");
